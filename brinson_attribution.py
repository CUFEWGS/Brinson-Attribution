...
import warnings
from wm import DateUtil

warnings.filterwarnings('ignore')

# wdir = os.path.dirname(__file__)
wdir = "D:/E/Python/Brinson-Attribution"
...
def get_trade_date(date):
    date = DateUtil.add_date(date, '.ap')
    return date
...
def brinson_attr_stock(weight, benchmark='000300.SH', freq='6M', version=2,
    ...
    if verbose:
        ...
        writer = pd.ExcelWriter(os.path.join(wdir, 'brinson_result_test',
                                f'{fund_code}_res.xlsx'))
        for i in brinson_single.keys():
            brinson_single[i].to_excel(writer, sheet_name=i)
        writer.save()
    ...
    for r in brinson_returns:
        for j in brinson_single.keys():
            dat_panel = brinson_single[j][r]
            if '总计' in dat_panel.index:
                dat_panel.drop(['总计'], inplace=True)
            single_ret.loc[j, r] = dat_panel.sum(axis=0)
...
def clean_fund_holding(save_ori=True):
    ...
    for f in files:
        try:
            dat = pd.read_csv(os.path.join(fund_dir, f),
                              encoding='utf-8', index_col=[0], parse_dates=True)
        except UnicodeDecodeError:
            dat = pd.read_csv(os.path.join(fund_dir, f),
                              encoding='gbk-8', index_col=[0], parse_dates=True)

        # dat = pd.read_csv(os.path.join(fund_dir, f), engine='c',
        #                   encoding='utf-8', index_col=[0], parse_dates=True)
        dat = dat.reset_index()
        dat['报告期'] = dat['报告期'].map(lambda d: str(d)[:10])
        dates = list(set(dat['报告期'].tolist()))
        del dat['序号']
        dat = dat.set_index([ '报告期'])
        writer = pd.ExcelWriter(os.path.join(fund_dir, f.split('持股')[0]+'.xlsx'))
        for i in dates:
            dat.loc[i, :].reset_index(drop=True).to_excel(writer, sheet_name=i, index=False)
        writer.save()
        # dat = dat.swaplevel(0, 1).to_dict()
        # dat.to_dict()
        # dat = {'1': pd.DataFrame(), '2': pd.DataFrame()}
